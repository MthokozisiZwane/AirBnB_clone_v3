#!/usr/bin/python3
'''
Creates a view for Amenity objects - handles RESTful API actions.
'''
...
def get_amenity(amenity_id):
    ...
    # Get an Amenity object with a given ID from storage
...
def delete_amenity(amenity_id):
    ...
    # Get Amenity object with the given ID from storage
    amenity = storage.get(Amenity, amenity_id)
    if amenity:
        # Deletes Amenity object from storage and saves changes
...
def update_amenity(amenity_id):
    ...
    amenity = storage.get(Amenity, amenity_id)
    if amenity:
        # Returns 400 error if the requested data is not in JSON format
        if not request.get_json():
            abort(400, 'Not a JSON')
        ...
        # Updates the attributes of the Amenity object with the JSON data
        for key, value in data.items():
            if key not in ignore_keys:
                setattr(amenity, key, value)

        # Saves the updated Amenity object to the storage
        amenity.save()
        # Returns the updated Amenity object in JSON format with 200 code
...
def bad_request(error):
    '''Returns Bad Request message for illegal requests made to the API.'''
