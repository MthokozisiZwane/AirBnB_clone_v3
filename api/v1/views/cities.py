#!/usr/bin/python3
'''
Create a new view for City and its Restful api
'''
...
def get_cities_by_state(state_id):
    ...
    #   a State and convert them to dictionaries
...
def create_city(state_id):
    ...
    city.save()
    # Returns newly created City object in JSON format with 201 status code
...
def update_city(city_id):
    ...
    '''
    # Get City object with the given ID from the storage
    city = storage.get(City, city_id)
    if city:
        # Check if the request data is in JSON format
        if not request.get_json():
